...
import argparse, os, subprocess, urllib.request, traceback, shutil, tarfile, gzip, threading, math
...
def program_execution_check(cmd):
        ...
        if cmderr.decode("utf-8") != '':
                print('Failed to execute program "' + cmd + '". Is this executable in the location specified/discoverable in your PATH, or does the executable even exist?')
                print('---')
...
def convert_hmm_db(hmmer3dir, hmmDbFileIn, hmmDbFileOut):
        # Format command
        cmd = os.path.join(hmmer3dir, 'hmmconvert') + ' ' + hmmDbFileIn + ' > ' + hmmDbFileOut
        # Run
...
def main():
        ...
        def validate_args(args):
                ...
                program_execution_check(os.path.join(args.hmmer3_dir, 'hmmbuild -h'))
                ...
                if args.superfamily != None or args.cath != None:
                        if not os.path.isfile(args.superfamily):
                                ...
                                quit()
                        elif not os.path.isfile(args.cath):
                                print('"' + args.cath + '" was not able to be located.')
                                print('Make sure you\'ve typed the file name or location correctly and try again.')
                                quit()
                # Obtain full paths for SUPERFAMILY/CATH to make later options easier
                args.superfamily = os.path.abspath(args.superfamily)
        ...
        # Declare hard-coded values
        '''The following tuple which may be useful later to tell if the user has
        been using the output directories for anything other than the program;
        it won't be entirely foolproof, but it should prevent major mistakes from
        occurring e.g., deleting necessary directory contents'''
        CDD_PREFIXES = ('cd', 'COG', 'KOG', 'LOAD', 'MTH', 'pfam', 'PHA', 'PRK', 'PTZ', 'sd', 'smart', 'TIGR', 'PLN', 'CHL')
        
        #### USER INPUT SECTION
        usage = """
        %(prog)s is designed to easily facilitate the conversion of the CDD and 
        SUPERFAMILY/CATH into a format compatible with HMMER 3.1+. It will
        attempt to automatically continue previously cancelled runs at the nearest
        completed step. This script is easily hackable to add or remove msa files
        in the cdd_extraction directory to alter the output .hmm file. This directory
        MUST be dedicated solely to this program; if this program encounters an error,
        certain directories and their contents will be removed so be warned.
        """
        # Required cmds
        p = argparse.ArgumentParser(description=usage)
        p.add_argument("output_directory", type = str, help="""Specify the name of the directory to download and process the
        ...
        # Optional cmds
        p.add_argument("-3", dest="hmmer3_dir", type = str, default = '',
        ...
        cddExtractDir = os.path.join(args.output_directory, 'cdd_extraction')
        cddHmmDir = os.path.join(args.output_directory, 'cdd_individual_models')
        spfamExtractDir = os.path.join(args.output_directory, 'superfamily_extraction')
        cathExtractDir = os.path.join(args.output_directory, 'cath_extraction')
        
        ### CDD HANDLING
        
        # Download
        if not os.path.isfile(os.path.join(args.output_directory, cddFileName)):
                print('# Downloading CDD files')
                file_dl(args.url, args.output_directory, cddFileName)
        ...
        if not os.path.isfile(os.path.join(cddExtractDir, 'cdd_untar.complete')):               # This should let us know if untarring was previously performed successfully while still being "hackable" by removing unwanted models from the directory
                try:
                        ...
                        untar(cddExtractDir, os.path.join(args.output_directory, cddFileName), False)     # We don't care about returning the updated file name from the untar function
                        ...
                        for file in tmpdirContents:
                                if not file.startswith(CDD_PREFIXES) and not file.lower().endswith('.fasta') and file != 'cdd_untar.complete':
        ...
        hmmbuild_dir(cddExtractDir, cddHmmDir, args.hmmer3_dir, args.threads)            # We can afford to spend a bit of time validating that the files are not 0kb (which is all we do if this step has been previously completed)
        ...
        dbFileName = os.path.join(args.output_directory, 'CDD.hmm')                               # If we don't add SUPERFAMILY/CATH to this file, this will be our final database name
        if not os.path.isfile(os.path.join(args.output_directory, 'cdd_hmm.complete')) or not os.path.isfile(os.path.join(args.output_directory, 'CDD.hmm')):
                concat_hmms(cddHmmDir, args.output_directory, 'CDD.hmm')
                create_blank_file(os.path.join(args.output_directory, 'cdd_hmm.complete'))
        else:
                print('## A "CDD.hmm" file is already present in outdir "' + args.output_directory + '". I won\'t overwrite this file.')
        ...
        if args.superfamily != None:
                if not args.superfamily.endswith('_3.1'):                               # If this already has _3.1 suffix, I'll assume someone already converted it using this program and are re-running this program using this file as an input
                        if not os.path.isfile(os.path.join(spfamExtractDir, 'supfam_conv.complete')) or not os.path.isfile(args.superfamily + '_3.1'):  # args.superfamily already has the full path leading to it
                                ...
                                convert_hmm_db(args.hmmer3_dir, args.superfamily, args.superfamily + '_3.1')
        ...
        if args.superfamily != None or args.cath != None:
                ...
                if not os.path.isfile(os.path.join(args.output_directory, 'added_concat.complete')) or not os.path.isfile(os.path.join(args.output_directory, concatFileName)):
                        concat_hmms(additionalHmms, args.output_directory, concatFileName)
                        create_blank_file(os.path.join(args.output_directory, 'added_concat.complete'))
                else:
                        print('## A "' + concatFileName + '" file is already present in outdir "' + args.output_directory + '". I won\'t overwrite this file.')
                        print('## Specify a new outdir, move the file elsewhere, or delete this file if it is outdated.')
                dbFileName = os.path.join(args.output_directory, concatFileName)                  # This will overwrite the database name from above
        
        ### PRESS FINAL FILE
        if not os.path.isfile(os.path.join(args.output_directory, 'press.complete')) or (not os.path.isfile(dbFileName + '.h3f') and not os.path.isfile(dbFileName + '.h3i') and not os.path.isfile(dbFileName + '.h3m') and not os.path.isfile(dbFileName + '.h3p')):
                print('# Running hmmpress to prepare database for use')
                hmmpress(dbFileName, args.hmmer3_dir)
                create_blank_file(os.path.join(args.output_directory, 'press.complete'))
