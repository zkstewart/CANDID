        ...
        with open(parsedFile, 'r') as fileIn:
                for line in fileIn:
                        ...
                        for entry in sl[1:]:
                                ...
                                if baseExtract:
                                        info[0] = os.path.basename(info[0])     # Sometimes, depending on how the database was made, we'll have full paths to domain IDs; we can optionally pull out just the base if that is appropriate here
                                for i in range(len(info)):
                                        info[i] = info[i].strip("'[]")          # Remove the formatting characters that are only there for visual inspection purposes
                                        # Change info type if relevant
                                        if i == 1 or i == 2:
                                                info[i] = int(info[i])
                                        elif i == 3:
                                                info[i] = float(info[i])
